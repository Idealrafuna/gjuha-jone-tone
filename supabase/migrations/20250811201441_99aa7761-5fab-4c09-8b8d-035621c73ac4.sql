-- Fix enum casting for dialect_type and re-run seed
-- Cities
...
-- Insert vocab + variants (cast dialect)
WITH lr AS (
  SELECT id FROM public.lessons WHERE slug = 'greetings-01'
), vrow AS (
...
SELECT id, 'gheg'::public.dialect_type, 'Tung', 'tuŋ', 'audio/lessons/hello_gheg_placeholder.mp3' FROM vrow
UNION ALL
SELECT id, 'tosk'::public.dialect_type, 'Përshëndetje', 'pərʃənˈdɛcjɛ', 'audio/lessons/hello_tosk_placeholder.mp3' FROM vrow;

-- Insert dialogues + lines (cast dialect)
WITH lr AS (
  SELECT id FROM public.lessons WHERE slug = 'greetings-01'
), drow AS (
...
VALUES
  ((SELECT id FROM drow), 1, 'A', 'gheg'::public.dialect_type, 'Tung! Si je?', 'tuŋ si jɛ', 'audio/lessons/line1_gheg_placeholder.mp3'),
  ((SELECT id FROM drow), 2, 'B', 'tosk'::public.dialect_type, 'Përshëndetje! Si je?', 'pərʃənˈdɛcjɛ si jɛ', 'audio/lessons/line2_tosk_placeholder.mp3');
